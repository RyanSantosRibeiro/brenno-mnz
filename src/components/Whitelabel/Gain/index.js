...
const Gain = () => {
    ...
    const active = (e) => {
        if($("#gain").offset().top - $(window).scrollTop() < (window.screen.height) && !$(".gain").hasClass("animate")) {
            $(".gain").addClass("animate");
        }
    }
    ...
    return  (
        <section className="gain" id='gain' style={{backgroundImage: `url(${image})`}}>
...
definidos no momento do contrato.</span></h2>
            ...
            {/* <div className="gain__highlight">
                <p className='gain__highlight__text'>Entre no <b>mundo virtual</b> e deixe sua marca ser descoberta por <span>milhões</span> de pessoas!</p>
            </div> */}
        </section>
    )
}
